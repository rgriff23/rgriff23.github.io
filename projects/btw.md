...
This is an R package for running BayesTraits from R (Mac OS only). The functions work by using `System` to run BayesTraits on your system and delete the output files after importing them into R.
...
Set the `.BayesTraitsPath` variable (this will likely be different on your machine).
...
Now let use `btw` to run some BayesTraits analyses. 

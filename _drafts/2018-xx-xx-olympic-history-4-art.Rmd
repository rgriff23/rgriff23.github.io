...
[COMPARE TO OVERALL TRENDS IN OLYMPIC PARTICIPATION]

The Art Competitions drew slightly more female competitors than other Olympic events during these years: 11.2% of Olympic Artists were female, as compared to 7.5% of all Olympians.
...
However, although 11.2% of artists were women, only 7% of medals were awarded to women, and just one gold medal.
...
art_women %>% filter(!is.na(Medal)) %>% arrange(Year) %>% print(n=Inf, width=Inf)
```
...
ggplot(medal_counts2, aes(x=Team, y=Count, fill=Medal)) +
  ...
  coord_flip() +
  scale_fill_manual(values=c("gold1","gray70","gold4")) +
  ggtitle("Nazi domination of Art Competitions at the Nazi Olympics") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
Maybe. But how many people would turn on their television for such a thing?
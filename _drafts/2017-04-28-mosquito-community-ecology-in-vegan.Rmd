...
It seems we have some problematic redundancy in our predictor variables. A principled way to choose which variable(s) to drop would be using the `drop1` function, which compares the change in AIC the results from dropping each variable from the full model:
...
As in the CA, we can see the field-forest gradient reflected along the first axis of variation (note that the direction of the gradient is arbitrary, so it doesn't matter that it has flipped compared to the CA).
...
The VIFs are acceptable.

I will not make the figures for the pCCA, which can be produced using the same code that we used for the CCA. If you do this on your own, you will find that not a whole lot has changed compared to the CCA. Compute the percent of variation explained by the predictors and test for signficance:
...
One of the most confusing things about ordination is figuring out how the different methods relate to one another and which one to use. The good news is that all of these methods tend to produce comparable results for a given dataset. You can always try running both `rda` and `cca` and see which approach yields better visualizations of your data. When I applied the `rda` family of methods to this mosquito data, I found that the ordination plots were difficult to read because they were very clumped in the middle with a few outliers. This makes sense since a few sites have very large abundances, and `rda` is sensitive to that. In contrast, `cca` works on relative values and produces ordination plots that are more spread out, so `cca` seemed like a better approach for this dataset. 

...
An alternative way to make more space on our plot is to display the sites and species on separate panels. Let's also add colors to our site labels to represent different habitats, as we did for the boxplots. 
...
One of the most confusing things about ordination is figuring out how the different methods relate to one another and which one to use. The good news is that all of these methods tend to produce comparable results for a given dataset. Choosing the best one for your data is as much art as science. You can always try running both `rda` and `cca` to see which approach yields better visualizations of your data, or try different ways of scaling the scores (see the`?plot.cca` help page and the `vegan` [design decisions document](https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=1&cad=rja&uact=8&ved=0ahUKEwj61YCQ4-rTAhWBKiYKHaaCCMkQFggsMAA&url=https%3A%2F%2Fcran.r-project.org%2Fweb%2Fpackages%2Fvegan%2Fvignettes%2Fdecision-vegan.pdf&usg=AFQjCNFr1yflIOHdjmqpmaI1eeXrAQMeFA&sig2=ALSGpFzLgtfGpbMubMKaaw) to read more about scaling options). The goal is simply to get a birds-eye view of the major patterns of similarity and dissimilarity across your dataset in as clear a way as possible. 

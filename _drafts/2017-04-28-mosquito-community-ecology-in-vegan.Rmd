...
Open R and load the packages we will need for this analysis, installing them first if you haven't already:
...
Import the data into R using the `fread` function from the `data.table` package. Let's also specify that `Habitat` is an ordered factor, which will be important for boxplots and linear models. 
...
It appears that species diversity increases as we move from the field to the forest. We can test for differences among habitats statistically using a linear model, with `Habitat` as an ordered categorical predictor of species diversity:
...
Now run the CCA and compute variance inflation factors (VIF) to check for redundancy in the data. As a rule of thumb, VIF > 10 indicates high levels of redundancy.
...
*For more information on species diversity indices, the `vegan` documentation is great and will point you toward any more specific literature you might want to look at. Try `?diversity`, `?rarefy`, `?renyi`.* 

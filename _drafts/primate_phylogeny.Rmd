...
description: "Add images, subplots, and other features"
...
knitr::opts_chunk$set(message=FALSE, warning=FALSE, comment=" ", cache=TRUE)
...
This plot is useful for identifying the node numbers necessary to label or draw attention to clades in the next section.
...
There are a couple of approaches we can take to delineate the clades of interest. One is to add shaded regions around them:

```{r post_2017-05_ggtree_clades1, fig.width=7, fig.height=7}
# highlight clades
p4 <- ggtree(tree) + 
  xlim(0, 125) +
...
Another would be to add labeled bars near the tips of the tree that show the extent of each clade (note how I adjust the text size and position for the small Tarsiidae family):

```{r post_2017-05_ggtree_clades2, fig.width=7, fig.height=7}
# label clades
p5 <- p4 + 
...
Finally, we can label our clades with images. For an elegant look, we can import images from the `phylopic` database, which `ggtree` is meant to work with. To annotate nodes with `phylopic`, first find the number of each node you want to annotate, then match each node with a link to an image on XXX. Here are the images I want to use for the major primate superfamilies:
...
```{r post_2017-05_ggtree_clades3, fig.width=7, fig.height=7}
# add phylopic images
...
```{r post_2017-05_ggtree_clades4, fig.width=7, fig.height=7}
...
In case you are wondering, that is in fact my winning team from Pokemon Blue Version in the early 2000s.
...
We can use `gheatmap` to display the data matrix:

```{r post_2017-05_ggtree_tips1, fig.width=7, fig.height=7}
# basic plot
p8 <- ggtree(tree) + 
  xlim(0, 125) +
...
```{r post_2017-05_ggtree_tips2, fig.width=7, fig.height=7}
# make traits binary
...
```{r post_2017-05_ggtree_tips3, fig.width=7.5, fig.height=4}
...
Now we will simulate a single continuous trait and perform ancestral state reconstructions (ASRs) and visualize the results across nodes. First, estimate the ASRs for the first continuous trait in our original data, using the `anc.Bayes` function from `phytools`:
...
```{r  post_2017-05_ggtree_asr1}
# plot continuous ASR posterior distributions with histograms

```

If we have ancestral state reconstructions for a discrete trait, ancestral states can be represented by a pie chart or bar chart depicting the relative probability of each state. Let's use XXX to reconstruct ancestral states for our first discrete character, and visualize the results with both pie charts and bar charts:

```{r post_2017-05_ggtree_asr2}
# make trait binary
trait.bi <- ifelse(trait > mean(trait), 1, 0)

# estimate discrete ASRs with stochastic character mapping


# plot discrete ASRs with pie charts and bar charts

```

### Concluding remarks

If you are in the business of plotting phylogenies, mastering `ggtree` is well worth the effort. The downsides are that there is a steeper learning curve if you are not already familiar with `ggplot`, and it can be a little harder to find help online just because it isn't as widely used alternatives (e.g., `plot.phylo` from the `ape` package). I note that [`phytools`](http://blog.phytools.org/) (which we used here to simulate data and reconstruct ancestral states) also has some interesting functions for visualizing phylogenetic comparative data and ancestral state reconstructions- see this [demo](https://lukejharmon.github.io/ilhabela/instruction/2015/07/05/plotting-methods/). However, as a general R toolkit for annotating phylogenies, `ggtree` is by far the most powerful and flexible option I am aware of.

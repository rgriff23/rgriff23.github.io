...
Now let's look at how NHL salaries are related to popularity. A sigmoidal curve seems to provide a decent enough fit to the data to be useful. The fitted curve is plotted along with the data below, and for some context, the vertical red line shows where Hilary Knight falls on this graph.
...
We can now use the fitted curve to estimate the average salary of NHL players with the same level of popularity as the USWNT players. To avoid the dangers of extrapolating beyond the range of the data, I focus on the 14 USWNT players with follower counts within the NHL range. Here are the estimated salaries- all 7 digit figures:   
...
# sort USWNT data and print with xtable
library(xtable)
print(xtable(uswnt.data[sort.int(predictions, index.return=T, decreasing=T)$ix,][1:14,-c(2,4)], align="llcc"), type="html", include.rownames = F, table.placement=)
```
...
*Twitter data was compiled in April, 2017.*
